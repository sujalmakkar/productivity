...
export default function Note(props) {
    ...
    useEffect(() => {
        if (fetchstate.current) {
            ...
            noteHeading ? noteHeading.addEventListener('paste', function (e) {
                ...
                const text = e.clipboardData ? (e.originalEvent || e).clipboardData.getData('text/plain') : // For IE
    ...
    return React.createElement(
        'div',
        { className: props.popup ? 'note popup box-shadow' : 'note box-shadow', 'data-id': props.id },
        React.createElement(
            'div',
            { className: 'note-contents-container' },
            React.createElement(
                'div',
                { className: 'note-text-container', onKeyDown: handlenote, onClick: handlenote },
                React.createElement('div', { className: 'note-heading text-container', ref: headingref, 'data-name': 'heading', 'data-id': props.id, suppressContentEditableWarning: 'true', contentEditable: 'true', 'aria-multiline': 'true', role: 'textbox', onKeyDown: handleHeading, onKeyUp: handleHeading, onClick: handleHeading }),
                React.createElement('div', { className: 'note-content text-container', ref: contentref, 'data-name': 'content', 'data-id': props.id, suppressContentEditableWarning: 'true', contentEditable: 'true', 'aria-multiline': 'true', role: 'textbox', onKeyDown: handleContent, onKeyUp: handleContent, onClick: handleContent })
            )
        ),
        React.createElement(
            'div',
            ...
            React.createElement(
                ...
                React.createElement(
                    ...
                    React.createElement(
                        'button',
                        { type: 'button', onClick: uploadImage },
                        React.createElement('img', { src: 'https://img.icons8.com/material-rounded/96/000000/add-image.png' })
                    )
                ),
                React.createElement(
                    'div',
                    ...
                    React.createElement(
                        ...
                        React.createElement('img', { src: 'https://img.icons8.com/material-outlined/96/000000/list.png' })
                    )
                )
            ),
            React.createElement(
                'div',
                { className: 'note-close', onClick: closeeditor, onMouseOver: saveEarly },
                React.createElement('img', { src: 'https://img.icons8.com/material-outlined/96/000000/delete-sign.png' })
            )
        )
    );
}